...
nltk.download('omw-1.4')  #Archivos para importar funciones de nltk.

lemmatizer = WordNetLemmatizer()

with open('intents.json', 'r', encoding='utf-8') as file:
    intents = json.load(file) #Abre el archivo de entrenamiento

words = []
classes = []
documents = []
ignore_letters = ['?', '!', '¿', '.', ','] #Hace excepciones que no afectan al entrenamiento 

#Clasifica los patrones y las categorías
for intent in intents['intents']: #Este ciclo for empieza a utilizar las listas que definimos antes
    for pattern in intent['patterns']: #Este otro ciclo accede a los patrones que implementamos en el entrenamiento
